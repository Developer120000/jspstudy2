...
@WebServlet("/Ex10_e")
public class Ex10_e extends HttpServlet {
...
//		out.println("<h2>현재 Ex10 페이지 입니다.");
//		out.println("<h3><ul>");
...
//		out.println("</ul></h3>");
		...
		//  response.sendRedirect("Ex10_2_e");      // 서블릿으로 이동 
		//  response.sendRedirect("view/ex09_d.jsp"); // html, jsp로 이동 
		
		// 포워드 : 다른 페이지로 이동하면서 기존의 request / response 정보를 가지고 간다.
		//        파라미터도 사용 가능하다.
		//        주소창에는 최초 주소가 보인다.
		// request.getRequestDispatcher("주소").forward(request, response);
		request.getRequestDispatcher("Ex10_2_e").forward(request, response);
